...
class OptGeoTarget_GMX(OptGeoTarget):
    def __init__(self, options, tgt_opts, forcefield):
        self.engine_ = GMX
        super(OptGeoTarget_GMX, self).__init__(options, tgt_opts, forcefield)

    def create_engines(self, engine_args):
        self.engines = OrderedDict()
        for sysname, sysopt in self.sys_opts.items():

            actual_gmx_top = sysopt.get('gmx_top', sysopt['topology'])

            self.engines[sysname] = self.engine_(
                target=self,
                name=sysname,
                gmx_top=actual_gmx_top,
                coords=sysopt['geometry'],
                **engine_args
            )
    ...
    def system_driver(self, sysname, save_mol=None):
        ...
        energy, rmsd, M_opt = engine.optimize(0)

        pos = M_opt.xyzs[0]

        if save_mol:
            M_opt.write(save_mol)

        return {
            'bonds': np.array([ic.value(pos) for ic in ic_dict['ic_bonds']]),
            'angles': np.array([ic.value(pos) * 180 / np.pi for ic in ic_dict['ic_angles']]),
            'dihedrals': np.array([ic.value(pos) * 180 / np.pi for ic in ic_dict['ic_dihedrals']]),
            'impropers': np.array([ic.value(pos) * 180 / np.pi for ic in ic_dict['ic_impropers']]),
        }
